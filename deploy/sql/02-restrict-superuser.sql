-- RENOVA — audit integrity: restrict Postgres superuser (Slice 15, AC7).
--
...
-- Manager. The app's own role (`renova`) is a least-privilege login, NOT a superuser.
--
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'renova') THEN
    CREATE ROLE renova LOGIN PASSWORD NULL;   -- password set out-of-band, never here
  END IF;
END$$;

ALTER ROLE renova NOSUPERUSER NOCREATEROLE NOCREATEDB NOBYPASSRLS;
