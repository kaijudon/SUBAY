# SUBAY — Slice 15 build-time deployment checklist
...
- [ ] **§1 App + secrets + pgcrypto**
  ...
  - [ ] `stat -c '%U %a' /etc/subay/subay.env` → `root 600`
...
- [ ] **§4 Updates / power / LUKS**
  - [ ] `50unattended-upgrades-subay` installed; `Automatic-Reboot "false"` confirmed
  - [ ] `subay-reboot-required.timer` enabled
...
- [ ] **§5 Backup + drill**
  - [ ] `subay-backup.timer` enabled; nightly run produces all 3 parts; secrets/key on separate `KEY_DEST` mount
  - [ ] `restore-drill.sh` passes; pgcrypto decrypts end-to-end; logged in RUNBOOK §5 table
- [ ] **§6 Monitoring**
  - [ ] `subay-healthcheck.timer` enabled; a forced failure pushes an alert
...
- [ ] **§7 Audit integrity**
  ...
  - [ ] `02-restrict-superuser.sql` applied; superuser = Data Manager only; app uses `subay` role
  - [ ] `subay-history-export.timer` enabled; exports hash-chained to object-lock off-site bucket
